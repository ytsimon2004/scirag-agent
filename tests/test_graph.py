"""Tests for scirag.graph.state — node functions tested in isolation."""
...
from scirag.graph.state import State, _entities_node, _retrieve_node, _synthesize_node
...
class TestEntitiesNode:
    def test_populates_entities_and_expanded_query(self):
        ...
        with patch("scirag.graph.state.extract_entities", return_value=entities), \
             patch("scirag.graph.state.expand_query", return_value="hippocampus place cells"):
...
class TestRetrieveNode:
    def test_uses_expanded_query_when_present(self):
        node = _mock_node()
        with patch("scirag.graph.state.retrieve", return_value=[node]) as mock_retrieve:
...
class TestSynthesizeNode:
    def test_calls_synthesize_with_query_and_nodes(self):
        nodes = [_mock_node()]
        with patch("scirag.graph.state.synthesize", return_value="Answer [1].") as mock_synth:
