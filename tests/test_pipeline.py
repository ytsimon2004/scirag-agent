...
Retrieval, entity extraction and index lookups are mocked; these tests lock the
relevance-gating and message-assembly contract that the CLI, UI and MCP server
all depend on.
"""
...
def _patch(*, entities=None, nodes=None, indexed=True):
    """Patch the three external collaborators of prepare_answer."""
    entities = entities if entities is not None else {"brain_region": ["CA1"]}
    nodes = nodes if nodes is not None else []
    return (
        patch.object(pipeline, "extract_entities", return_value=entities),
        patch.object(pipeline, "expand_query", side_effect=lambda q, e: q),
...
def _run(**kw):
    p_ent, p_exp, p_ret, p_idx = _patch(**kw)
    with p_ent, p_exp, p_ret, p_idx:
...
class TestGating:
    ...
    def test_empty_index_skips_retrieval(self):
        p_ent, p_exp, p_ret, p_idx = _patch(indexed=False)
        with p_ent, p_exp, p_ret as mock_retrieve, p_idx:
...
class TestMessageAssembly:
    def test_history_is_inserted_between_system_and_user(self):
        ...
        p_ent, p_exp, p_ret, p_idx = _patch(nodes=[_make_node("1", 0.9)])
        with p_ent, p_exp, p_ret, p_idx:
